...
class Room extends EventEmitter {
  ...
  constructor(localParticipant, signaling, options) {
    ...
    Object.defineProperties(this, {
      ...
      _subscriberHintsSignaling: {
        value: options.tracks.find(({ kind, name }) => kind === 'data' && name === 'subscriber_hints')
      },
      dominantSpeaker: {
        enumerable: true,
        get() {
...
function connectParticipant(room, participantSignaling) {
  const { _log: log, _clientTrackSwitchOffControl: clientTrackSwitchOffControl, _contentPreferencesMode: contentPreferencesMode, _subscriberHintsSignaling: subscriberHintsSignaling, localParticipant } = room;
  const participant = new RemoteParticipant(participantSignaling, { log, clientTrackSwitchOffControl, contentPreferencesMode, localParticipantSid: localParticipant.sid, subscriberHintsSignaling });
  ...
  const eventListeners = [
    ...
    function reemit() {
      ...
      if (event === 'trackMessage') {
        const [/* 'trackMessage */, hintStr, track] = args;
        if (track.name === 'subscriber_hints') {
          const hint = JSON.parse(hintStr);
          const publication = room.localParticipant.videoTracks.get(hint.track);
          if (publication) {
            publication.setSubscriberHint(hint);
          }
        }
      }
    }
