...
class Participant extends EventEmitter {
  ...
  constructor(signaling, options) {
    ...
    Object.defineProperties(this, {
      ...
      _localParticipantSid: {
        value: options.localParticipantSid
      },
      _log: {
        value: log
      },
      ...
      _subscriberHintsSignaling: {
        value: options.subscriberHintsSignaling
      },
      _tracks: {
        value: tracks
      },
  ...
  _handleTrackSignalingEvents() {
    const { _log: log, _clientTrackSwitchOffControl: clientTrackSwitchOffControl, _contentPreferencesMode: contentPreferencesMode, _localParticipantSid: localParticipantSid, _subscriberHintsSignaling: subscriberHintsSignaling } = this;
    const self = this;
    ...
    function trackSignalingSubscribed(signaling) {
      ...
      const setRenderHint = renderHint => {
        ...
        // NOTE(mmalavalli): Send the first message after one second in case the
        // signaling LocalDataTrack is not published yet.
        if (!sentOnce) {
          setTimeout(() => {
            subscriberHintsSignaling.send(JSON.stringify({
              hint: renderHint,
              subscriber: localParticipantSid,
              track: sid
            }));
            sentOnce = true;
          }, 1000);
